    ...
    <div className="space-y-3">
      ...
      >
        ...
        <CardContent className="px-4 pb-4 -mt-8">
          <Avatar className="h-16 w-16 border-3 border-card shadow-md">
          ...
          <h3 className="font-semibold text-foreground mt-2 text-base leading-tight hover:underline">{fullName}</h3>
          <p className="text-sm text-muted-foreground mt-0.5 line-clamp-2">{title}</p>
...
function StatRow({ icon, label, value, border }: {
  ...
  return (
    <div className={`flex items-center justify-between px-4 py-2.5 hover:bg-muted cursor-pointer transition-colors ${border ? 'border-t border-border' : ''}`}>
