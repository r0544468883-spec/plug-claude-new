...
const FREELANCE_HR_STEPS: TourStep[] = [
  ...
  },
  {
    section: 'hr-tools',
    targetSelector: '[data-tour="hr-tools-hub"]',
    titleHe: 'סינון AI + פרסום רב-ערוצי',
    titleEn: 'AI Screening + Multi-board Publishing',
    descriptionHe: 'הגדר כללי סינון חכמים שמדרגים מועמדים אוטומטית, ופרסם משרות ל-10+ לוחות בלחיצה אחת. שניהם נגישים ישירות ממשרה פתוחה.',
    descriptionEn: 'Set smart screening rules that auto-rank candidates, and publish jobs to 10+ boards in one click. Both accessible directly from an open job.',
    icon: Megaphone,
  },
  {
    section: 'hr-tools',
    targetSelector: '[data-tour="hr-tools-hub"]',
    titleHe: 'תבניות הצעות עבודה + תאימות EEO',
    titleEn: 'Offer Templates + EEO Compliance',
    descriptionHe: 'ספריית תבניות להצעות עבודה, NDA, חוזי קבלן ועוד. לוח תאימות EEO/OFCCP עוקב אחרי יחס גיוון ודוח 4/5.',
    descriptionEn: 'Template library for offers, NDAs, contractor agreements & more. EEO/OFCCP compliance dashboard tracks diversity ratios and 4/5ths rule.',
    icon: Shield,
  },
  {
    section: 'hr-tools',
    targetSelector: '[data-tour="hr-tools-hub"]',
    titleHe: 'קמפיינים + הודעות רב-ערוציות',
    titleEn: 'Nurture Campaigns + Multi-Channel Messaging',
    descriptionHe: 'בנה רצפי Drip לטיפוח מועמדים, ושלח הודעות ב-SMS, WhatsApp ואימייל מתיבת דואר אחת.',
    descriptionEn: 'Build drip sequences to nurture candidates, and send messages via SMS, WhatsApp & email from a unified inbox.',
    icon: Send,
  },
  {
    section: 'hr-tools',
    targetSelector: '[data-tour="hr-tools-hub"]',
    titleHe: 'סורסינג חיצוני + אנליטיקס חזויה',
    titleEn: 'External Sourcing + Predictive Analytics',
    descriptionHe: 'חפש מועמדים ב-LinkedIn, GitHub ו-Stack Overflow וייבא אותם ל-PLUG. דשבורד חזויה חוזה Time-to-Hire ומזהה צווארי בקבוק.',
    descriptionEn: 'Source candidates from LinkedIn, GitHub & Stack Overflow and import to PLUG. Predictive dashboard forecasts Time-to-Hire and spots bottlenecks.',
    icon: TrendingUp,
  },
  {
    section: 'hr-tools',
    targetSelector: '[data-tour="hr-tools-hub"]',
    titleHe: 'דשבורד מותאם + תמלול AI',
    titleEn: 'Custom Dashboard + AI Transcription',
    descriptionHe: 'בנה דשבורד אישי עם 12 סוגי ווידג\'טים. תמלול AI אוטומטי לראיונות וידאו עם סיכום וייצוא.',
    descriptionEn: 'Build a personal dashboard with 12 widget types. Auto AI transcription for video interviews with summary and export.',
    icon: Blocks,
  },
  {
    section: 'missions',
...
export function RecruiterTour({ currentSection, onNavigate }: RecruiterTourProps) {
  ...
  const getTransitionTip = (from: DashboardSection, to: DashboardSection): string => {
    const tips: Record<string, string> = {
      ...
      'post-job->hr-tools': isHebrew ? '🛠️ בוא נגלה את כלי ה-HR החדשים!' : '🛠️ Let\'s discover the new HR tools!',
      'hr-tools->missions': isHebrew ? '🎯 הנה לוח הפרויקטים!' : '🎯 Here\'s the project billboard!',
