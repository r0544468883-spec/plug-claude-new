...
serve(async (req) => {
  ...
  try {
    ...
    if (triggerType === "weekly_free") {
      ...
      if (existing) {
        ...
        const { data: jobDetails } = await supabase
          .from("jobs")
          .select("id, title, description, requirements, location, salary_range, job_type, field, is_active, created_at")
          .in("id", jobIds);
    ...
    const { data: allJobs } = await supabase
      .from("jobs")
      .select("id, title, description, requirements, location, salary_range, job_type, field, experience_level_id, is_active, created_at")
      .eq("is_active", true)
    ...
    const preFiltered = allJobs
      ...
      .map((job: any) => {
        ...
        if (preferredFields.length > 0) {
          factors += 40;
          if (job.field && preferredFields.includes(job.field)) score += 40;
        }
