...
serve(async (req) => {
  ...
  try {
    ...
    const { candidateId, applicationId, force } = await req.json();
    ...
    // Cache-first: skip AI call if a fresh summary exists for this application
    // and was generated by the current model. The caller can pass `force: true`
    // to bypass (e.g., HR explicitly regenerating).
    if (!force) {
      const { data: existing } = await supabaseService
        .from('applications')
        .select('ai_candidate_summary, ai_candidate_summary_model, ai_candidate_summary_generated_at, updated_at')
        .eq('id', applicationId)
        .single();

      if (
        existing?.ai_candidate_summary &&
        existing?.ai_candidate_summary_model === CURRENT_AI_MODEL_VERSION &&
        existing?.ai_candidate_summary_generated_at &&
        existing?.updated_at &&
        new Date(existing.ai_candidate_summary_generated_at) >= new Date(existing.updated_at)
      ) {
        return new Response(
          JSON.stringify({ success: true, summary: existing.ai_candidate_summary, cache_hit: true }),
          { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
    }
...
Return JSON with: skills (array of top 5), soft_skills (array of top 3), salary_positioning ("below_market" | "at_market" | "above_market"), summary (2-3 sentences), strengths (array of 3). Return ONLY valid JSON, no markdown, no code blocks.`;
    ...
    await supabaseService
      .from('applications')
      .update({
        ai_candidate_summary: summary,
        ai_candidate_summary_model: CURRENT_AI_MODEL_VERSION,
        ai_candidate_summary_generated_at: new Date().toISOString(),
      } as any)
      .eq('id', applicationId);

    return new Response(JSON.stringify({ success: true, summary, cache_hit: false }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (error) {
